(function( $ ) {
    ...
    function build_file($element, options) {
        ...
        $element.on("change",  function(){
            ...
            else $(this).closest('.ce-upload').find('.upl-filename').text( $(this).val() );
        });
        $element.on({
            mouseenter: function(){
    ...
    function build_select($element, options) {
            ...
            var selWrapper = $('<div />', {
                'class': selWrapperClass
                    ...
                    $('<div />', {
                        ...
                        /*'html': $(this).find('.selected').html( $(this).clone() )*/
                    }).append(
                            $('<span />', {
                                'class': 'ceSelect-arr'
                            })
            ...
            if( $(this).is('optgroup') ){
                ...
                $(this).find('option').each(function(index){
                    if( $(this).is(':selected') ) {
                        ...
                        $(this).find('.selected').html( $(this).clone()).append( $(this).parents('.ceSelect').find('.ceSelect-selected') );
                    }
		...
		if( options.selectAutoWidth ) {
            $ceSelect.css({
                width:  $element.next().css('width', '100%')
            });
		}

		$element.focus(function(){
            $ceSelect.addClass('open focus');
            $ceSelect.removeClass('closed');
		});
		$element.blur(function(){
            $ceSelect.addClass('closed');
            $ceSelect.removeClass('focus open');
		});
		// click events
			...
			if( !$(this).parent().hasClass('disabled')) {
                if(!$(this).parent().hasClass('multi')) {
                    ...
                    $(document).bind('click', function(e) {
                        ...
                        if (!firstClick && x != $('.ceSelect-selected') && !$(x).parents('.ceSelect-selected').size()) {
                            $ceSelect.addClass('closed');
                            $ceSelect.removeClass('focus open');
		...
		$ceSelect.on("click", ".ceSelect-item", function(){
			if( !$ceSelect.hasClass('disabled') ){
				if( !$ceSelect.hasClass('multi') ){
					...
					$ceSelect.find('.ceSelect-selected').html( $(this).clone()).append('<span class="ceSelect-arr"></span>');
                    $ceSelect.addClass('closed');
                    $ceSelect.removeClass('focus open');
  ...
  $.fn.customElements = function(method) {
    if ( methods[method] ) {
        ...
        $.error( 'Метод "' +  method + '" не найден в плагине jQuery.customElements' );
    }
  };
})(jQuery);